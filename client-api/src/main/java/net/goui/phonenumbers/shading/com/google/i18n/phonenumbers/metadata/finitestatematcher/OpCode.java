...
package net.goui.phonenumbers.shading.com.google.i18n.phonenumbers.metadata.finitestatematcher;
...
public enum OpCode {
  ...
  BRANCH(0) {
    @Override
    State execute(DigitSequenceMatcher.DataView data, DigitSequenceMatcher.DigitSequence ignored) {
  ...
  SINGLE(1) {
    @Override
    State execute(DigitSequenceMatcher.DataView data, DigitSequenceMatcher.DigitSequence in) {
      int op = data.readByte();
      if (!in.hasNext()) {
   ...
   */
  private static State map(DigitSequenceMatcher.DataView data, DigitSequenceMatcher.DigitSequence in, State noInputState) {
    int op = data.readInt();
    if (!in.hasNext()) {
      return noInputState;
    }
  ...
  abstract State execute(DigitSequenceMatcher.DataView data, DigitSequenceMatcher.DigitSequence in);
}
